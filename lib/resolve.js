...
async function block(name = '', skip = 0) {
	...
	const find = async (ext, fallback = true) => {
		const
		...
		for (let i = skip; i < r.length; i++) {
			...
			const
				components = `/**/${file}`,
				virtualComponents = !baseExt && `/**/${name}.${ext}`,
		...
		for (let i = 0; i < rgxpDependencies.length; i++) {
			...
			const
				components = `/**/${file}`,
				virtualComponents = !baseExt && `/**/${realName}.${ext}`,
				mask = [path.join(root, components)].concat(virtualComponents ? path.join(root, virtualComponents) : []);
		...
		return baseExt || skip ? null : fallback ? path.join(baseRoot, name) : null;
	};

	let res = await find('index.ts', false) ?? await find('index.js');
...
function blockSync(name = '', skip = 0) {
	...
	let
		res = find('index.ts', false) ?? find('index.js');
