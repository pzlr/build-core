...
$C(config.dependencies).forEach((obj) => {
	...
	if (fs.existsSync(cfg)) {
		...
		const
			...
			depsPath = path.join(base, cfg.blockDir),
			serverDepsPath = path.join(base, cfg.serverDir);

		dependencies.push(depsPath);
		serverDependencies.push(serverDepsPath);
		rootDependencies.push(base);

		depMap[baseSrc].blocksPath = depsPath;
		depMap[baseSrc].serverDepsPath = serverDepsPath;
		depMap[baseSrc].rootDepsPath = base;
		...
		const
			...
			depsPath = path.join(base, config.blockDir),
			serverDepsPath = path.join(base, config.serverDir);
...
/**
 * Returns dependency record if url is inside it's path
 * or undefined otherwise
 *
 * @param {string} url
 * @returns {Object|undefined}
 */
function depByFilePath(url) {
	return $C(depMap).one.get((value) => isPathInside(url, value.path));
}
