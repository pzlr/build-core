...
$C(config.dependencies).forEach((obj) => {
	...
	if (fs.existsSync(cfg)) {
		...
		const
			...
			serverPath = path.join(base, cfg.serverDir);

		dependencies.push(depsPath);
		serverDependencies.push(serverPath);
		rootDependencies.push(base);

		depMap[baseSrc].path = depsPath;
		depMap[baseSrc].serverPath = serverPath;
		depMap[baseSrc].rootPath = base;
		...
		const
			...
			serverPath = path.join(base, config.serverDir);
