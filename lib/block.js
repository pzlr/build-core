...
class Block {
	...
	static async getAll(names) {
		const
			...
			graphLockFile = path.join(projectCWD, 'components-graph.json');

		const projectHash = objectHash().hash({
			srcHash: hashFiles.sync({files: [path.join(resolve.sourceDir, '/**/*')], algorithm: 'md5'}),
			...
			if (fs.existsSync(graphLockFile)) {
				const normalizedJSONReviver = (key, val) => {
					...
					if (Sugar.Object.isString(val) && path.extname(val) !== '') {
						return path.resolve(path.dirname(graphLockFile), val);
					}
				...
				const
					fromCache = fs.readJSONSync(graphLockFile, {reviver: normalizedJSONReviver});
		...
		fs.writeFileSync(graphLockFile, JSON.stringify(cacheData, jsonReplacer, 2));
