...
class Block {
	...
	static async getAll(names) {
		const
			cachedBlockMap = createSerializableMap(),
			sources = [await resolve.cwd, ...resolve.rootDependencies];
		...
		for (let i = sources.length; i--;) {
			const
				file = path.join(sources[i], 'graph-lock.json');

			if (!fs.existsSync(file)) {
				break;
			}

			$C.extend(false, cachedBlockMap, fs.readJSONSync(file, {reviver: jsonReviver}));
			sources.pop();
		}

		if (sources.length === 0) {
			return $C(cachedBlockMap).map((block) => {
				Object.setPrototypeOf(block, blockProto);
				return block;
			});
		}
		...
		if (!names) {
			...
			const dests = [await resolve.block(), ...resolve.dependencies]
				.slice(0, sources.length);

			names = await $C(dests.slice(0, sources.length))
				.async
				.to(new Map())
		...
		const
			mixedBlockMap = $C.extend({traits: true}, createSerializableMap(), blockMap, cachedBlockMap);

		const normalizedBlockMap = await $C(mixedBlockMap).async.map(async (block, key) => {
			if (!blockMap.has(key)) {
				Object.setPrototypeOf(block, blockProto);
				return block;
			}
			...
			return {
				...
				parent: block.parent,
				resolvedParent: await block.getParent({cache: mixedBlockMap}),

				dependencies: block.dependencies,
				resolvedDependencies: await block.getDependencies({cache: mixedBlockMap}),
				resolvedOwnDependencies: await block.getDependencies({cache: mixedBlockMap}),

				libs: block.libs,
				resolvedLibs: await block.getLibs({cache: mixedBlockMap}),
				resolvedOwnLibs: await block.getLibs({onlyOwn: true, cache: mixedBlockMap}),
				runtimeDependencies: await block.getRuntimeDependencies({cache: mixedBlockMap}),

				type: block.type,
				mixin: block.mixin,

				index: await block.index,
				logic: await block.logic,
				styles: await block.styles,
				tpl: await block.tpl,
				etpl: await block.etpl,

				__proto__: blockProto
			};
		});

		fs.writeFileSync(path.join(await resolve.cwd, 'graph-lock.json'), JSON.stringify(normalizedBlockMap));
