...
class Block {
	...
	static async get(name) {
		const decl = new Declaration(await getFile(
		...
		if (decl.mixin) {
			...
			while ((parent = resolve.block(this.name, skip))) {
				const
					parentDecl = new Declaration(await getFile(path.join(parent.result, 'index.js')));
