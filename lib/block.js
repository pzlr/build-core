...
class Block {
	 ...
	 */
	static async get(name, file = 'index.js') {
		const
			blockPath = await resolve.block(path.join(name, file)),
	...
	static async getAll(names) {
		...
		const blocks = await $C(names)
			...
			.reduce(async (res, el) => {
				const i = res.push([]) - 1;
				await $C([].concat(await el)).parallel().forEach(async ([nm, fm], j) => {
					if (set[nm]) {
						return;
					}
