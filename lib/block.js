...
class Block {
	...
	/**
	 * Returns components-lock cache from filesystem by specific filepath
	 *
	 * @param filepath - path to components-lock file
	 * @returns {Map<string, !Block>}
	 */
	static getCacheFromPath(filepath) {
		const normalizedJSONReviver = (key, val) => {
			val = jsonReviver(key, val);

			if (Sugar.Object.isString(val) && path.extname(val) !== '') {
				return path.resolve(path.dirname(filepath), val);
			}

			if (Sugar.Object.isObject(val) && val.declaration != null) {
				Object.setPrototypeOf(val, Block.prototype);
			}

			return val;
		};

		return fs.readJSONSync(filepath, {reviver: normalizedJSONReviver});
	}

	/**
	 * Returns path to component-lock file
	 *
	 * @param lockPrefix - prefix of components-lock file
	 * @returns {string}
	 */
	static getComponentsLockPath(lockPrefix) {
		const
			projectCWD = resolve.cwd;

		return path.join(
			projectCWD,
			[].concat(lockPrefix || [], 'components-lock.json').join('.')
		);
	}
	...
	static async getAll(names, opts = {}) {
		const
			...
			componentsLockPath = this.getComponentsLockPath(opts.lockPrefix);
		...
		try {
			if (fs.existsSync(componentsLockPath)) {
				const fromCache = this.getCacheFromPath(componentsLockPath);

				if (fromCache.hash === projectHash) {
		...
		fs.writeFileSync(componentsLockPath, JSON.stringify(cacheData, jsonReplacer, 2));
