...
class Block {
	...
	static async get(name) {
		...
		async function getDecl(url) {
			return new Declaration(getFile(path.join(await url, 'index.js')));
		}
	...
	static async getAll(names) {
		if (!names) {
			...
			names = await $C([await resolve.block(), ...resolve.dependencies]).parallel().reduce(async (map, root) => {
				const src = [
				...
				if (!map[src]) {
					map[src] = $C(
						vinyl.src(src, {read: false}).pipe(through.obj((file, enc, cb) => {
							file.base = root;
							file.nm = path.dirname(file.relative);
							cb(null, file);
						}))
					).async.map((file) => file.nm);
				}
		...
		const blocks = await $C(names).parallel().reduce(
			async (res, el) => {
				const i = res.push([]) - 1;
				await $C([].concat(await el)).parallel().forEach(async (nm, j) => {
					if (set[nm]) {
						return;
					}
		...
		return $C(Sugar.Array.flatten(blocks)).reduce((map, decl) => map.set(decl.name, decl), new Map());
	}
	...
	async getRuntimeDependencies({cache} = {}) {
		...
		return Object.assign(await get(this.name), {libs: await this.getLibs({cache})});
	}
}
