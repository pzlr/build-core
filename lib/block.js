...
class Block {
	...
	static async getAll(names) {
		const
			...
			sources = [await resolve.cwd, ...dependencies];
		...
		const blockMap = await $C(Sugar.Array.flatten(blocks)).to(createSerializableMap()).reduce((map, decl) => {
		...
		fs.writeFileSync(path.join(await resolve.cwd, 'graph-lock.json'), JSON.stringify(blockMapToSave, undefined, 2));
