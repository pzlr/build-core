...
class Block {
	...
	static async getAll(names) {
		...
		const projectHash = objectHash().hash({
			srcHash: hashFiles.sync({files: glob.sync(path.join(sourceDirs, '/**/index.js'), globOpts).sort()}),
			projectFiles: glob.sync(path.join(sourceDirs, '/**/*.@(js|ts|styl|ss)'), globOpts)
				.sort()
		...
		try {
			if (fs.existsSync(componentsLockFile)) {
				...
				const
					fromCache = fs.readJSONSync(componentsLockFile, {reviver: normalizedJSONReviver});
		...
		const normalizedBlockMap = await $C(blockMap).async.to(createSerializableMap()).reduce(async (map, decl, key) => {
			map.set(key, {
				...decl,
				...
				libs: decl.libs,
				resolvedLibs: await decl.getLibs({cache: blockMap}),
				resolvedOwnLibs: await decl.getLibs({onlyOwn: true, cache: blockMap}),
