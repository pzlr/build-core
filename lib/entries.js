	...
	const entries = await $C(await glob(path.join(entry(), '*.js'))).parallel().reduce(
		async (res, src) => {
			...
			res[name] = {
				...
				get parent() {
					return $C(getEntryParents(source)).one.get();
				},

				get parents() {
					return getEntryParents(source);
				},
	...
	function factory(entries) {
		...
		function dependencies() {
			if (!dependenciesCache) {
				...
				const down = (entry, name) => {
					res = new Set([name, ...res]);

					$C(entry.parents).reverse.forEach((name) => {
						down(entries[name], name);
					});
				};

				$C(entries).forEach((entry, name) => {
					down(entry, name);
				});

				dependenciesCache = [...res];
			}
