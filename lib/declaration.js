...
function package(name) {
	...
	const dependencies = (...names) => {
		${resName}.dependencies = names.flatMap((el) => el).filter((el) => el != null);
		return {libs};
	};

	const libs = (...libs) => {
		${resName}.libs = libs.flatMap((el) => el).filter((el) => el != null);
		return {dependencies};
	};
...
`;
		...
		declarationsCache[declaration] = Function(code);
		return new this(declarationsCache[declaration]());
	}
