...
class Declaration {
	...
	static parse(declaration, test = false) {
		...
		if (declarationsCache[declaration]) {
			return declarationsCache[declaration];
		}
...
`;
		...
		declarationsCache[declaration] = new this(Function(code)());
		return declarationsCache[declaration];
	}
	...
	constructor(declaration) {
		...
		this.name = name;
		this.type = validators.blockTypes[name.charAt(0)];
		this.parent = parent;
		this.mixin = mixin;
		this.dependencies = dependencies;
		this.libs = libs;

		[this, this.dependencies, this.libs].forEach(Object.freeze);
	}
